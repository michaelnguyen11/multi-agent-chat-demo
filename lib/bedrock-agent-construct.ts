...
export class BedrockKbConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string) {
    ...
    const assetsPath = path.join(__dirname, "../S3_storage/docs/");
    const assetDoc = s3deploy.Source.asset(assetsPath);

    const assetsTsPath = path.join(__dirname, "../S3_storage/code/typescript/");
    const assetTsDoc = s3deploy.Source.asset(assetsTsPath);

    const assetsPyPath = path.join(__dirname, "../S3_storage/code/python/");
